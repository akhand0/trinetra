...
Open [http://localhost:3000](http://localhost:3000). The canvas requires a live ClickHouse connection backed by OpenTelemetry data.
...
- `OPENROUTER_API_KEY` and `TRINETRA_MODEL` (defaults to Kimi K3 through
...
Open [http://localhost:3000](http://localhost:3000) for the single agent page. The real Trigger.dev conversation and its typed telemetry panels stay inside this page without navigating elsewhere.

### Live OpenTelemetry Demo telemetry

Ingest live telemetry from the ~20-microservice
...
`logs`/`metrics`/`spans` names:

```bash
clickhouse-client --multiquery < db/clickhouse/005_otel_views.sql
```

Manufacture incidents from the demo's flagd UI
(`paymentServiceFailure`, `cartServiceFailure`, `adServiceHighCpu`, …).
...
```text
app/                  Next.js routes, live stream, rewards, Trigger actions
